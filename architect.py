...
class Architect(object):
  ...
  def __init__(self, model, args):
    ...
    self.optimizer = torch.optim.Adam(self.model.arch_parameters(),
        lr=args.arch_learning_rate, betas=(0.5, 0.999), weight_decay=args.arch_weight_decay)# 网络结构参数
  ...
  def _compute_unrolled_model(self, input, target, eta, network_optimizer):
    ...
    try:
      moment = _concat(network_optimizer.state[v]['momentum_buffer'] for v in self.model.parameters()).mul_(self.network_momentum)# 增加动量
    except:
      moment = torch.zeros_like(theta)# 不加动量
  ...
  def step(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer, unrolled):
    # self.optimizer = torch.optim.Adam(self.model.arch_parameters(),
    #                                   lr=self.arch_learning_rate, betas=(0.5, 0.999),
  ...
  def _backward_step_unrolled(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer):
    ...
    implicit_grads = self._hessian_vector_product(vector, input_train, target_train) #计算公式八(dαLtrain(w+,α)-dαLtrain(w-,α))/(2*epsilon)   其中w+=w+dw'Lval(w',α)*epsilon w- = w-dw'Lval(w',α)*epsilon

    for g, ig in zip(dalpha, implicit_grads):# 公式六减公式八 dαLval(w',α)-(dαLtrain(w+,α)-dαLtrain(w-,α))/(2*epsilon)
      g.data.sub_(eta, ig.data)

    for v, g in zip(self.model.arch_parameters(), dalpha):#对α进行更新
      if v.grad is None:
  ...
  def _hessian_vector_product(self, vector, input, target, r=1e-2):
    ...
    for p, v in zip(self.model.parameters(), vector):#dαLtrain(w+,α)
